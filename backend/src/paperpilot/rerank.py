...
def rerank_documents(query: str, documents: list[str], top_k: int) -> list[tuple[int, float]]:
    ...
    if not documents:
        return []
    identity: list[tuple[int, float]] = [(i, 0.0) for i in range(min(top_k, len(documents)))]
    if not settings.enable_rerank:
        return identity
    try:
        result = _get_client().rerank(query, documents, model=settings.rerank_model, top_k=top_k)
